...
def verificar_respuesta(pregunta_actual: dict, datos_juego: dict, respuesta: int) -> bool:
    ...
    if datos_juego["doble_chance_activa"]:
        if respuesta != correcta and datos_juego["ya_fallo_una"] == False:
            datos_juego["ya_fallo_una"] = True
            return False

        datos_juego["doble_chance_activa"] = False
        datos_juego["ya_fallo_una"] = False

    if respuesta == correcta:
        puntos = 100

        if datos_juego["comodin_x2_activo"]:
            puntos *= 2
...
def vincular_respuestas_a_botones(lista_respuestas, pregunta_actual):
    respuestas = [
        pregunta_actual["respuesta_1"],
        pregunta_actual["respuesta_2"],
        pregunta_actual["respuesta_3"],
        pregunta_actual["respuesta_4"]
    ]

    correcta_idx = pregunta_actual["respuesta_correcta"] - 1

    for i in range(4):
        lista_respuestas[i]["texto"] = respuestas[i]
        lista_respuestas[i]["es_correcta"] = (i == correcta_idx)
    ...
    if (type(lista_respuestas) == list and type(pos_click) == tuple and type(datos_juego) == dict and type(lista_preguntas) == list and type(pregunta_actual) == dict and len(lista_respuestas) > 0):
        ...
        for i in range(len(lista_respuestas)):
            if lista_respuestas[i]["rectangulo"].collidepoint(pos_click):
                ...
                doble = datos_juego.get("doble_chance_activa", False)
                ya_fallo = datos_juego.get("ya_fallo_una", False)
                if doble:

                    if respuesta != correcta and not ya_fallo:

                        datos_juego["ya_fallo_una"] = True

                        lista_respuestas[respuesta]["visible"] = False
                        ...
                        if datos_juego.get("comodin_x2_activo", False):
                            puntos_extra = 2
                            datos_juego["comodin_x2_activo"] = False
                verificar_respuesta(pregunta_actual,datos_juego,respuesta + 1)
                datos_juego["doble_chance_activa"] = False
                datos_juego["ya_fallo_una"] = False
...
def mostrar_pregunta_pygame(pregunta_actual: dict, pantalla: pygame.Surface, cuadro_pregunta: dict, lista_respuestas: list) -> bool:
    ...
    for respuesta in lista_respuestas:
        if not respuesta.get("visible", True):
            continue

        mostrar_texto(respuesta["superficie"],respuesta["texto"],(20, 20),FUENTE_ARIAL_20_NEGRITA,COLOR_NEGRO)
        pantalla.blit(respuesta["superficie"], respuesta["rectangulo"])
...
def crear_comodines(textura,x,y,cantidad_botones):
    nombres = ["bomba","x2","doble_chance","pasar"]
    lista_comodines = []
    for i in range(cantidad_botones):
        boton = crear_elemento_juego(textura,150,ALTO_BOTON,x,y)
...
def activar_comodin(nombre: str, datos_juego: dict, pregunta_actual: dict, lista_respuestas: list):
    comodines = datos_juego.get("comodines", {})

    if comodines[nombre]["usado"]:
        return False

    if nombre == "bomba":
        incorrectas = []
        for i in range(len(lista_respuestas)):
            if lista_respuestas[i]["es_correcta"] == False:
